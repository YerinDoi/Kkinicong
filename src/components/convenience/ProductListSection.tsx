...
interface ProductListSectionProps {
  products: Product[];
  keyword?: string;
  onSwipePrev?: () => void;   // ★ 부모가 넘기면 실제 데이터 전환
  onSwipeNext?: () => void;   // ★ 부모가 넘기면 실제 데이터 전환
}
...
const slide: Variants = {
  enter: (d: 1 | -1) => ({ x: d === 1 ? 14 : -14, opacity: 0, transition: TWEEN_FAST }),
  center: { x: 0, opacity: 1, transition: SPRING },
  exit:   (d: 1 | -1) => ({ x: d === 1 ? -10 : 10, opacity: 0, transition: TWEEN_FAST }),
};
...
export default function ProductListSection({
  ...
  // ---- 스와이프 감지 (down/up, cancel 포함) ----
  const start = useRef<{ x: number; t: number } | null>(null);
  const swipedRef = useRef(false);
  const [dir, setDir] = useState<1 | -1>(1);
  const [swipeNonce, setSwipeNonce] = useState(0); // ★ 목록이 안 바뀔 때도 애니메이션 재생 용
  ...
  const finish = (dx: number, dt: number) => {
    ...
    if (dx <= -DIST || (fast && dx < 0)) {
      ...
      else setSwipeNonce(n => n + 1); // ★ 데이터 안 바뀌어도 피드백
  ...
  useEffect(() => {
    ...
    const id = setTimeout(() => (swipedRef.current = false), 180);
        ...
        >
          ...
          <div className="flex items-center gap-2 min-w-0 flex-1">
            <div className="overflow-hidden min-w-0 h-[20px] flex items-center">
              <AnimatePresence mode="sync" initial={false} custom={dir}>
                <motion.span
                  ...
                  exit="exit"
                  custom={dir}
                  className="inline-block align-baseline leading-[20px]  
                   text-black text-body-bd-title font-semibold truncate will-change-transform"
                >
                  {p.name}
                </motion.span>
              </AnimatePresence>
            ...
            <div className="">
              <AnimatePresence mode="sync" initial={false} custom={dir}>
                <motion.span
                  key={`time-${p.id}-${swipeNonce}`}         
                  variants={slide}
                  initial="enter"
                  animate="center"
                  exit="exit"
                  custom={dir}
                  className="inline-block text-body-md-description text-[#919191] will-change-transform"
                >
                  {p.relativeCreatedAt}
                </motion.span>
              </AnimatePresence>
          ...
          {/* 오른쪽: 상태 뱃지 (텍스트만 좌우 슬라이드) */}
          <div className="overflow-hidden w-[86px] text-right">
            <AnimatePresence mode="sync" initial={false} custom={dir}>
              <motion.span
                key={`avail-${p.id}-${swipeNonce}`}          // ✅ 텍스트만 재키잉
                variants={slide}
                initial="enter"
                animate="center"
                exit="exit"
