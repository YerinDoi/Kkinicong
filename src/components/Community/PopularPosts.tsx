...
const PopularPosts = () => {
  ...
  useEffect(() => {
    axiosInstance.get("/api/v1/community/post/popular")
      .then((res) => {
            ...
            {posts.map((post, idx) => (
              <SwiperSlide key={`${post.id}-${idx}`}>
                <div className="h-[39px] w-full flex  px-[12px]">
                  ...
                  >
                    <span className="text-[#029F64] font-semibold">{idx + 1}.</span> 
                    <span className="text-[#919191] font-regular text-body-md-description ">{post.title}</span>
                  </button>
                </div>
              </SwiperSlide>
