...
const FavoriteStoreCard = ({ count, onClick }: FavoriteStoreCardProps) => (
  <button
    onClick={onClick}
    className="flex items-center w-full px-[20px] py-[16px] rounded-[16px] gap-[24px] border border-main-color bg-white self-stretch"
  >
    {/* 아이콘 */}
    ...
    <div className="flex flex-col flex-1 items-start gap-[4px] font-pretendard">
      <span className="text-[16px] font-semibold leading-[20px] text-text-gray">지금까지 찜한 가게 수</span>
      <span className="text-[20px] font-semibold leading-[28px] text-sub-color">{count}개</span>
    </div>
